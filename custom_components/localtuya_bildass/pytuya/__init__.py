...
def pack_message(msg, hmac_key=None):
    ...
    if prefix == PREFIX_6699_VALUE:
        ...
        # For v3.5, use random 12-byte IV
        import os
        iv = msg.iv if msg.iv else os.urandom(12)
        aad = header_data[4:]  # AAD is header without prefix
        _LOGGER.debug("pack_message 6699: key=%s, iv=%s, aad=%s, raw_len=%d, cmd=%d",
                     hmac_key.hex(), iv.hex(), aad.hex(), len(raw), msg.cmd)
        ...
        _LOGGER.debug("pack_message 6699: encrypted=%s (len=%d)", encrypted.hex(), len(encrypted))
...
def unpack_message(data, hmac_key=None, header=None, no_retcode=False, logger=None):
    ...
    if header.prefix == PREFIX_6699_VALUE:
        ...
        # Extract payload (header.length bytes after header)
        # Structure: [header 18B][payload=IV+encrypted+tag][suffix 4B]
        # payload_len (header.length) includes: IV(12) + encrypted_data + tag(16)
        payload = data[header_len : header_len + header.length]

        # Suffix is AFTER payload, not inside it
        suffix_bytes = data[header_len + header.length : header_len + header.length + 4]
        suffix = struct.unpack(">I", suffix_bytes)[0]

        # Tag is last 16 bytes of payload
        tag = payload[-16:]
        payload_without_tag = payload[:-16]

        # Extract IV (first 12 bytes of payload)
        iv = payload_without_tag[:12]
        encrypted_payload = payload_without_tag[12:]

        # Decrypt using GCM
        if logger:
            logger.debug(
                "unpack_message 6699: iv=%s, tag=%s, encrypted_len=%d, key=%s, raw_encrypted=%s",
                iv.hex() if iv else None,
                tag.hex() if tag else None,
                len(encrypted_payload),
                hmac_key.hex() if hmac_key else None,
                encrypted_payload.hex() if encrypted_payload else None
            )
        ...
        # Method 1: GCM with AAD (standard)
        try:
            cipher = AESCipher(hmac_key)
            if logger:
                logger.debug("unpack_message 6699: trying GCM with AAD=%s", aad.hex())
            ...
            crc_good = True
            if logger:
                logger.debug("unpack_message 6699: GCM+AAD SUCCESS, decrypted=%r", decrypted[:48] if len(decrypted) > 48 else decrypted)
        except Exception as gcm_ex:
            if logger:
                logger.debug("unpack_message 6699: GCM+AAD FAILED: %s", gcm_ex)

        # Method 2: GCM without AAD (some devices)
        if decrypted is None:
            try:
                cipher = AESCipher(hmac_key)
                if logger:
                    logger.debug("unpack_message 6699: trying GCM without AAD")
                decrypted = cipher.decrypt(
                    encrypted_payload,
                    use_base64=False,
                    decode_text=False,
                ...
                crc_good = True
                if logger:
                    logger.debug("unpack_message 6699: GCM no AAD SUCCESS, decrypted=%r", decrypted[:48] if len(decrypted) > 48 else decrypted)
            except Exception as gcm_noaad_ex:
                if logger:
                    logger.debug("unpack_message 6699: GCM no AAD FAILED: %s", gcm_noaad_ex)

        # Method 3: CTR mode fallback (no tag verification)
        if decrypted is None:
            try:
                cipher = AESCipher(hmac_key)
                if logger:
                    logger.debug("unpack_message 6699: trying CTR fallback")
                decrypted = cipher.decrypt(
                    encrypted_payload,
                    use_base64=False,
                    decode_text=False,
                ...
                crc_good = True
                if logger:
                    logger.debug("unpack_message 6699: CTR fallback SUCCESS, decrypted=%r", decrypted[:48] if len(decrypted) > 48 else decrypted)
            except Exception as ctr_ex:
                if logger:
                    logger.debug("unpack_message 6699: CTR fallback also FAILED: %s", ctr_ex)
                crc_good = False
                decrypted = encrypted_payload

        # Extract retcode from decrypted payload if present
        # Session key negotiation commands (3, 4, 5) don't have retcode - payload is raw data
        retcode = 0
        if no_retcode is False and header.cmd not in (SESS_KEY_NEG_START, SESS_KEY_NEG_RESP, SESS_KEY_NEG_FINISH):
...
class MessageDispatcher(ContextualLogger):
    ...
    def __init__(self, dev_id, listener, protocol_version, local_key, enable_debug, real_local_key=None):
        ...
        self.real_local_key = real_local_key if real_local_key else local_key
    ...
    def add_data(self, data):
        ...
        while self.buffer:
            ...
            # For session negotiation messages, always use real_local_key (device key)
            # because local_key may not be set yet or may be session key
            if header.cmd in (SESS_KEY_NEG_START, SESS_KEY_NEG_RESP, SESS_KEY_NEG_FINISH):
                hmac_key = self.real_local_key if self.version >= 3.4 else None
            else:
...
class TuyaProtocol(asyncio.Protocol, ContextualLogger):
    ...
    def __init__(
        ...
        self.dps_cache = {}
        import os
        self.local_nonce = os.urandom(16)  # random 16 bytes for session negotiation
        self.remote_nonce = b""
    ...
    def _setup_dispatcher(self, enable_debug):
        ...
        return MessageDispatcher(
            ...
            real_local_key=self.real_local_key
        )
    ...
    async def exchange(self, command, dps=None):
        ...
        if self.version >= 3.4 and self.real_local_key == self.local_key:
            ...
            # Give device time to process session key before sending commands
            if self.version == 3.5:
                self.debug("v3.5: sending HEART_BEAT to verify session key")
                # Send heartbeat first to verify session key works
                try:
                    hb_result = await self.exchange_quick(MessagePayload(HEART_BEAT, b""), 2)
                    self.debug("v3.5: HEART_BEAT response: %r", hb_result)
                except Exception as e:
                    self.debug("v3.5: HEART_BEAT failed: %s", e)
    ...
    async def _negotiate_session_key(self):
        ...
        if not rkey:
            self.debug("session key negotiation failed on step 1: no response")
            return False
        if not isinstance(rkey, TuyaMessage):
            self.debug("session key negotiation failed on step 1: response is not TuyaMessage, got %r", type(rkey))
            return False
        self.debug("session key negotiation step 1 response: cmd=%d, payload_len=%d, payload=%r",
                   rkey.cmd, len(rkey.payload), rkey.payload[:64] if len(rkey.payload) > 64 else rkey.payload)
        if len(rkey.payload) < 48:
            self.debug("session key negotiation failed on step 1: payload too short (%d < 48)", len(rkey.payload))
            return False
        ...
        # For Protocol 3.5 (6699 format), payload is already decrypted by unpack_message (GCM)
        # For Protocol 3.4 (55AA format), we need to decrypt here (ECB)
        if self.version == 3.5 and rkey.prefix == PREFIX_6699_VALUE:
            self.debug("v3.5 session key response already decrypted by GCM, payload=%r", payload)
        else:
            try:
                # Protocol 3.4 - decrypt with ECB
        ...
        self.debug("session key negotiation step 2 payload: %r (len=%d)", payload, len(payload))

        # Skip retcode (4 bytes) if present at start
        if payload[:4] == b'\x00\x00\x00\x00':
            self.debug("Skipping 4-byte retcode from payload")
            payload = payload[4:]

        if len(payload) < 48:
            self.debug("session key negotiation step 2 failed, too short response after retcode skip (len=%d < 48)", len(payload))
            return False

        # Nonce is 16 bytes - use directly for XOR
        # Both local_nonce (b"0123456789abcdef") and remote_nonce are 16 ASCII chars
        # XOR them directly as bytes, don't convert hex strings to raw bytes!
        self.remote_nonce = payload[:16]
        self.debug("Remote nonce: %r (raw bytes: %s)", self.remote_nonce, self.remote_nonce.hex())

        hmac_check = hmac.new(self.local_key, self.local_nonce, sha256).digest()
        received_hmac = payload[16:48]

        if hmac_check != received_hmac:
            self.debug(
                "session key negotiation step 2 HMAC mismatch (may be ok for some devices): wanted=%r but got=%r",
                binascii.hexlify(hmac_check),
                binascii.hexlify(received_hmac),
            )

        self.debug("Session key calc: local_nonce=%s, remote_nonce=%s, device_key=%s",
                   self.local_nonce.hex(), self.remote_nonce.hex(), self.local_key.hex() if isinstance(self.local_key, bytes) else self.local_key)
        rkey_hmac = hmac.new(self.local_key, self.remote_nonce, sha256).digest()

        # Calculate encrypted session key BEFORE sending SESS_KEY_NEG_FINISH
        xor_result = bytes(
            [a ^ b for (a, b) in zip(self.local_nonce, self.remote_nonce)]
        )
        self.debug("Session nonce XOR'd: %s", xor_result.hex())

        cipher = AESCipher(self.real_local_key)
        if self.version == 3.5:
            # Protocol 3.5: Session key = AES-GCM encrypt XOR result, ciphertext only
            # IV = local_nonce[:12] (our nonce, first 12 bytes)
            from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
            from cryptography.hazmat.backends import default_backend

            miv = self.local_nonce[:12]
            self.debug("v3.5 session key: XOR=%s, IV=%s, device_key=%s",
                      xor_result.hex(), miv.hex(), self.real_local_key.hex())

            # Raw AES-GCM: encryptor.update() returns ONLY ciphertext (no IV prefix)
            ...
            # encrypted is 16 bytes ciphertext (no IV, no tag)
            session_key = encrypted[:16]
            self.debug("v3.5 GCM ciphertext: %s (len=%d), session_key=%s",
                      encrypted.hex(), len(encrypted), session_key.hex())

            # SESS_KEY_NEG_FINISH payload: ONLY HMAC (32 bytes)
            # Device calculates the same session_key independently
            self.debug("Sending SESS_KEY_NEG_FINISH: HMAC=%s (32 bytes only)",
                      rkey_hmac.hex())
            await self.exchange_quick(MessagePayload(SESS_KEY_NEG_FINISH, rkey_hmac), None)

            self.local_key = self.dispatcher.local_key = session_key
            self.debug("Session key negotiate success (v3.5 GCM)! session key: %s", self.local_key.hex())
        else:
    ...
    def _encode_message(self, msg):
        ...
        if self.version == 3.5:
            ...
            # Session negotiation ALSO uses 6699/GCM format for true v3.5 devices
            if msg.cmd in (SESS_KEY_NEG_START, SESS_KEY_NEG_RESP, SESS_KEY_NEG_FINISH):
                # Session negotiation for v3.5 uses 6699/GCM format
                hmac_key = self.real_local_key
                prefix = PREFIX_6699_VALUE  # 6699 for v3.5 session negotiation
                # Payload stays unencrypted here - pack_message will encrypt with GCM
                self.debug("v3.5 session negotiation using 6699/GCM format, cmd %r", msg.cmd)
            else:
